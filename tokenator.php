...
*/
  ...
  class Tokenator {
     ...
     *
     * @param string $apikey The apikey bound to token (32 chars max).
     * @param string $userid The user id bound to token (32 chars max).
     *
     * @return token|string The token generated by the function.
     */
    public static function generate($apikey, $userid) {
      ...
      if(!(mysqli_num_rows($result) == 0)) { // Make sure that by some chance, the token is not a duplicate of one that is already in the database.
        ...
        $sql = "INSERT INTO `".self::$DB_name."`.`tokens` (`ID`, `token`, `timestamp`, `apikey`, `userid`) VALUES (NULL, '".$token."', NULL, '".$apikey."', '".$userid."')"; // Add the token to the DB.
        $result = mysqli_query($mysql, $sql) or die(mysqli_error($mysql));
        $mysql->close();
        return $token;
      }
     ...
     * @param int $days Number of days this token is expected to be valid.
     *
     * @return true if valid or false if invalid
     */
    public static function validate($token, $days=10) {
      
      $mysql = mysqli_connect(self::$DB_host, self::$DB_user, self::$DB_pass, self::$DB_name);
      $sql = "SELECT `token`, unix_timestamp(`timestamp`) as `timestamp`, `apikey`, `userid` FROM `tokens` WHERE `token` = '".$token."'"; // Finds if there is a token match in the database.
      $result = mysqli_query($mysql, $sql) or die(mysqli_error($mysql));
      if(mysqli_num_rows($result) >= 1) { // If there is a matching token...
        $rarr=mysqli_fetch_array($result);
        $diff_secs = time() - intval($rarr['timestamp']);
        $diff_days = $diff_secs / (60 * 60 * 24);

        // is token expired
        if($days < $diff_days){
          $sql = "DELETE FROM `tokens` WHERE `token` = '".$token."'"; // Delete the token from the database, it has been expired and is no longer needed.
          $result = mysqli_query($mysql, $sql) or die(mysqli_error($mysql));
          $mysql->close();
          return false;
        }
